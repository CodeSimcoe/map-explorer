...
public class PlutoExplorer extends Application {
    ...
    public void start(final Stage primaryStage) {
        ...
        this.reset(imageView, width / 2, height / 2);

        ObjectProperty<Point2D> mouseDown = new SimpleObjectProperty<>();

        imageView.setOnMousePressed(e -> {

            Point2D mousePress = this.imageViewToImage(imageView, new Point2D(e.getX(), e.getY()));
            mouseDown.set(mousePress);
        });

        imageView.setOnMouseDragged(e -> {
            Point2D dragPoint = this.imageViewToImage(imageView, new Point2D(e.getX(), e.getY()));
            this.shift(imageView, dragPoint.subtract(mouseDown.get()));
            mouseDown.set(this.imageViewToImage(imageView, new Point2D(e.getX(), e.getY())));
        });
        ...
        imageView.setOnScroll(e -> {
            ...
            Point2D mouse = this.imageViewToImage(imageView, new Point2D(e.getX(), e.getY()));
            ...
            imageView.setOnMouseClicked(e -> {
                ...
                this.reset(imageView, width, height);
            }
        });

        Hyperlink link = new Hyperlink("Image Credit: NASA/JHUAPL/SwRI");
        link.setOnAction(e -> this.getHostServices().showDocument(IMAGE_CREDIT_URL));

        link.setPadding(new Insets(10));
        link.setTooltip(new Tooltip(IMAGE_CREDIT_URL));

        HBox buttons = this.createButtons(width, height, imageView);
    ...
    private HBox createButtons(final double width, final double height, final ImageView imageView) {
        Button reset = new Button("Reset");
        reset.setOnAction(e -> this.reset(imageView, width / 2, height / 2));
        Button full = new Button("Full view");
        full.setOnAction(e -> this.reset(imageView, width, height));
    ...
    private void reset(final ImageView imageView, final double width, final double height) {
        imageView.setViewport(new Rectangle2D(0, 0, width, height));
    }

    // shift the viewport of the imageView by the specified delta, clamping so
    // the viewport does not move off the actual image:
    private void shift(final ImageView imageView, final Point2D delta) {
        Rectangle2D viewport = imageView.getViewport();
    ...
    private Point2D imageViewToImage(final ImageView imageView, final Point2D imageViewCoordinates) {
